// src/components/forms/skill-form.tsx
'use client';
...
export default function SkillForm() {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            await loadSkillsData(); // Refresh checklist skill terbaru dari database

            setUploadSuccessMessage(
                `Berhasil mendeteksi ${result.extractedSkills.length} skill dari CV Anda: ${result.extractedSkills.join(', ')}`
            );
        } catch (error: any) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* PANEL UNGGAH CV PDF (AKSEN DASHED HIJAU EMERALD) */}
                ...
                >
                    ...
                    {uploading ? (
                        <div className="flex flex-col items-center space-y-2">
                            ...
                            <p className="text-sm font-medium text-green-700">Model AI sedang mengekstrak CV...</p>
                        </div>
                    ) : (
                        <>
                            ...
                            <p className="text-xs text-gray-400">Hanya berkas PDF (Maksimal ukuran 2MB)</p>
                        </>
                    )}
                </div>
            </div>

            {/* GRID DAFTAR CHECKBOX MANUAL */}
